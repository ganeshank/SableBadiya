...
public class AdminHomeController extends HttpServlet {
    ...
    private static final String DISTRIBUTE_DATA = "/distribute_data";
    private static final String ORDER_FILTER = "/order_filter";
    private static final String ORDER_FILTER_DETAILS = "/order_filter_details";
    private static final String ADD_ITEM = "/add_item";
    private static final String ADD_ADDRESS_FOR_ORDER = "/add_address_for_order";
    private static final String SEARCHED_ADDRESS = "/search_address";
    ...
    private static final String DISPATCH_SEND_MESSAGE = "/jsp/send_message.jsp";
    private static final String DISPATCH_DISTRIBUTE_DATA = "/jsp/distribute_data.jsp";
    private static final String DISPATCH_ORDER_FILTER = "/jsp/order_filter.jsp";
    private static final String DISPATCH_ORDER_FILTER_DETAILS = "/jsp/order_filter_details.jsp";
    private static final String DISPATCH_ADD_ADDRESS_FOR_ORDER = "/jsp/add_address_for_order.jsp";
    private static final String DISPATCH_SEARCHED_ADDRESS = "/jsp/existing_address.jsp";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
			...
			if (pathInfo == null || pathInfo.equalsIgnoreCase(HOME_DEFAULT_VIEW)) {
				...
				request.setAttribute("categories", categoryService.getAllCategory(con, false, false, false, false));
			...
			}else if(pathInfo.equalsIgnoreCase(SEND_MESSAGE)){
				
				request.getRequestDispatcher(DISPATCH_SEND_MESSAGE).forward(request, response);
			}else if(pathInfo.equalsIgnoreCase(DISTRIBUTE_DATA)){
				
				request.getRequestDispatcher(DISPATCH_DISTRIBUTE_DATA).forward(request, response);
			}else if(pathInfo.equalsIgnoreCase(ORDER_FILTER)){
				
				HttpSession session = request.getSession();
				session.removeAttribute("offline-order");
				request.getRequestDispatcher(DISPATCH_ORDER_FILTER).forward(request, response);
			}else if(pathInfo.equalsIgnoreCase(ADD_ITEM)){
				
				addItem(request, response);
			}else if(pathInfo.equalsIgnoreCase(ADD_ADDRESS_FOR_ORDER)){
				request.getRequestDispatcher(DISPATCH_ADD_ADDRESS_FOR_ORDER).forward(request, response);
			}else if(pathInfo.equalsIgnoreCase(SEARCHED_ADDRESS)){
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try{
			...
			uploadPath  = uploadPath + File.separator + categoryVo.getCategoryMediaForder();
	        
			String uploadAtApp = uploadPath + File.separator + "app";
	        ...
	        File uploadDirForApp = new File(uploadAtApp);
	        if (!uploadDirForApp.exists()) {
	        	uploadDirForApp.mkdir();
	        }
            ...
            String filePathApp = uploadAtApp + File.separator + fileName;
            File storeFileApp = new File(filePathApp);
          
            fileItemForUpload.write(storeFileApp);
	...
	private void updatePriceRedirect(HttpServletRequest request, HttpServletResponse response){
		try{
			...
			CategoryService categoryService = new CategoryServiceImpl();
			List<CategoryVo> categoryVos = categoryService.getAllCategory(con, true, false, false, false);
	...
	private void disptachToOrderFilterDetails(HttpServletRequest request, HttpServletResponse response){
		try{
			HttpSession session = request.getSession();
			CartDetails cartDetails = (CartDetails) session.getAttribute("offline_order");
	...
	private void addItem(HttpServletRequest request, HttpServletResponse response){
		try{
			...
			cartItem.setPrice(new BigDecimal(total));
			cartItem.setUom(goodsVo.getUom());
			cartItem.setGoodsVo(goodsVo);
			
			HttpSession session = request.getSession();
			
			CartDetails cartDetails = (CartDetails) session.getAttribute("offline-order");
			
			if(cartDetails==null){
				cartDetails = new CartDetails();
				cartDetails.setSubtotalAmount(cartItem.getPrice());
				cartDetails.setTotalAmount(cartItem.getPrice());
				cartDetails.setTotalMsrp(cartItem.getPrice());
				
				cartDetails.setCartItems(new ArrayList<CartItem>());
			}else{
				cartDetails.setSubtotalAmount(cartDetails.getSubtotalAmount().add(cartItem.getPrice()));
				cartDetails.setTotalAmount(cartDetails.getTotalAmount().add(cartItem.getPrice()));
				cartDetails.setTotalMsrp(cartDetails.getTotalMsrp().add(cartItem.getPrice()));
			}
			
			List<CartItem> cartItems = cartDetails.getCartItems();
			cartItem.setTotalOrderAmount(cartDetails.getTotalAmount());
			cartItems.add(cartItem);
			
			cartDetails.setCartItems(cartItems);
			
			session.setAttribute("offline-order", cartDetails);
			
			response.setContentType("application/json");
	        response.getWriter().write(new Gson().toJson(cartItem));
